...
with st.sidebar:
    ...
    **The goal:** Help you make informed decisions by considering multiple viewpoints before forming opinions.
    ...
    5. **Discover** authoritative sources presenting opposing views
    ...
    **All processing happens locally. Your data never leaves your machine.**
    ...
    st.markdown("---")
    st.caption("Powered by Whisper + Llama 3 via Ollama")
    st.caption("100% Local • 100% Private • 100% Open Source")
    ...
    if not video_url:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                st.info(f"**Topic:** {data.get('topic_summary', 'No summary available.')}")
                st.caption(f"Overall Sentiment: **{data.get('overall_sentiment', 'N/A').upper()}**")
                
                # =============================================================================
                # COUNTER-ARGUMENTS & SOURCES
                # =============================================================================
                st.markdown('<div class="section-header">🔓 Counter-Perspectives</div>', unsafe_allow_html=True)
                st.markdown("""
                These are **opposing viewpoints** the algorithm likely isn't showing you. 
                Each is backed by academic context and verified sources.
                """)
                ...
                if counter_arguments:
                    for counter in counter_arguments:
                        ...
                        if suggested_videos:
                            ...
                            for video in suggested_videos[:3]:  # Show top 3
                                ...
                                if relevance >= 0.85:
                                    ...
                                    badge_class, badge_text = "relevance-low", "MODERATE RELEVANCE"
                                ...
                                <div class="video-card">
                                    {'<img src="' + video.get('thumbnail', '') + '" class="video-thumbnail" alt="Video thumbnail">' if video.get('thumbnail') else ''}
                                    <div class="video-content">
                                        <div class="video-title">{video.get('title', 'Untitled Video')}</div>
                                        <div class="video-meta">
                                            📺 {video.get('channel_name', 'Unknown Channel')} • 
                                            ⏱️ {duration_str} • 
                                            👁️ {views_str}
                                        </div>
                            ...
                            st.info("⚠️ No high-quality sources found for this specific counter-perspective.")
                        
                        st.markdown("<br>", unsafe_allow_html=True)
                
                else:
                    st.warning("No counter-arguments generated. This may indicate the video presents multiple balanced perspectives already.")
                ...
                st.caption("JSON export includes all claims, counter-arguments, sources, and metadata.")
            
            else:
                st.error(f"❌ Error: {response.status_code} - {response.text}")
        
        except requests.exceptions.Timeout:
            st.error("⏱️ Request timed out. The video may be too long or processing is taking longer than expected.")
        except requests.exceptions.ConnectionError:
            st.error("🔌 Cannot connect to backend. Ensure the API is running: `uvicorn api.main:app --reload`")
...
<div style="text-align: center; color: #64748b; padding: 2rem;">
    ...
    <p style="font-size: 0.75rem; color: #475569;">
        100% Local Processing • Privacy-First • Open Source<br>
        Powered by Whisper + Llama 3 via Ollama
    </p>
</div>
""", unsafe_allow_html=True)